// InstructorProfile.js

import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { useParams } from 'react-router-dom';

const InstructorProfile = () => {
    const { id } = useParams();

   const avatar = "data:image/jpeg;base64,";
    
    const [instructorData, setInstructorData] = useState({
        instru_name: '',
        instru_email: '',
        instru_phone: '',
        avatar: 'https://via.placeholder.com/150', // Placeholder image URL
        instru_about: '',
        instru_specialization: '',
        instru_qualification: '',
    });

    useEffect(()=>{
        axios.post('http://localhost:5000/getinstructorprofile',{id})
        .then(instructor => {
            setInstructorData(instructor.data);
            console.log("Instructor Data: ", instructor.data);
        })
        .catch(err => console.log("Error fetching instructor data: ", err));
    }, []);

    return (
        <div className="container mx-auto mt-8">
            <div className="max-w-md mx-auto bg-white rounded-lg overflow-hidden shadow-md">
                <div className="p-4">
                    <div className="text-center">
                        <img
                            src={avatar}
                            alt="Instructor Avatar"
                            className="w-32 h-32 mx-auto object-cover rounded-full"
                        />
                    </div>
                    <p className="text-gray-700 font-semibold">Instructor Name:</p>
                    <h2 className="text-xl font-semibold mt-4">{instructorData.instru_name}</h2>
                    <p className="text-gray-700 font-semibold">Instructor Email:</p>
                    <p className="text-gray-600">{instructorData.instru_email}</p>
                    {/* <p className="text-gray-700 font-semibold">Instructor Phone:</p>
                    <p className="text-gray-600">{instructorData.instru_phone}</p> */}
                    <div className="mt-4">
                        <p className="text-gray-700 font-semibold">About Me:</p>
                        <p className="text-gray-600">{instructorData.instru_about}</p>
                    </div>
                    <div className="mt-4">
                        <p className="text-gray-700 font-semibold">Instructor Specialization:</p>
                        <p className="text-gray-600">{instructorData.instru_specialization}</p>
                    </div>
                    <div className="mt-4">
                        <p className="text-gray-700 font-semibold">Instructor Qualification:</p>
                        <p className="text-gray-600">{instructorData.instru_qualification}</p>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default InstructorProfile;
